"""
this is a copy of theano.compile.nanguardmode with the slight change of
ignoring nan's if the class of a node is theano.sandbox.rng_mrg.GPU_mrg_uniform

see https://groups.google.com/forum/#!topic/pylearn-users/4E7xYRCJ-7I for more
on the issue

see HACK comments for the 2 changed lines
"""
...
class NanGuardMode(Mode):
    ...
    def __init__(self, nan_is_error, inf_is_error, big_is_error=True):
        ...
        def do_check_on(var, nd, f, is_input):
            ...
            error = False
            # HACK
            if isinstance(nd.op, theano.sandbox.rng_mrg.GPU_mrg_uniform):
                return
            if nan_is_error:
                err = False
