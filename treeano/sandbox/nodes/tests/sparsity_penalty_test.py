...
def test_elementwise_sparsity_penalty_node_serialization():
    tn.check_serialization(sp.ElementwiseSparsityPenaltyNode("a"))


def test_auxiliary_sparsity_penalty_node_serialization():
    tn.check_serialization(sp.AuxiliarySparsityPenaltyNode("a"))


def test_elementwise_sparsity_penalty_node1():
    network = tn.SequentialNode(
        "s",
        [tn.InputNode("i", shape=(5, 3)),
         sp.ElementwiseSparsityPenaltyNode("sp", sparsity=0.1)]
    ).network()
...
def test_elementwise_sparsity_penalty_node2():
...
def test_auxiliary_sparsity_penalty_node():
    ...
    network = tn.HyperparameterNode(
        ...
        tn.SequentialNode(
            ...
            [tn.InputNode("i", shape=(10, 3)),
             tn.DenseNode("d", num_units=9),
             sp.AuxiliarySparsityPenaltyNode("scp", cost_reference="sum"),
             sp.ElementwiseSparsityPenaltyNode("sp"),
             tn.AggregatorNode("a"),
