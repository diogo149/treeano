...
- for cost:
  - SendToNode
  - SumNode
- network.get_all_inputs
- multidensenode
- have container node return variable that throws exception AssertOp
...
- test
  - add test for node w/ generated children: create, serialize, deserialize, make sure same
  - unit test that before and after building a network, the __dict__ of a node is the same
