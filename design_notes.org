...
- RNNs
  - recurrent.py
    - add SimpleRecurrentNode
    - ScanNode recurrent architectures
    - add in normal LSTM/GRU/SRN
  ...
  - test: create recurrent net that loads weights from treeano net, and assert results are exactly the same
- make wrapper around T.nnet.categorical_crossentropy
  - assert dtype of ytrue == int32, and ndim == 1
- ScalarSumNode
- rename inits.Normal to NormalWeightInitialization
- DenseCombineNode
  - SplitCombine + LinearMappingNode each + combine with sum + BiasNode
- test mnist vs lasagne - should have same performance
- goals:
...
- how can nodes pass hyperparameters into their children the right way?
  - overwriting get_hyperparameter is wrong, because it affects all children
  - ideally we want to apply it only to the correct children
- canopy.transformations.
  - remove_dropout
    - replaces dropout with identity
  - remove_node
    - replaces node with single child
  - remove_subtree
    - replaces node with identity
  - remove_parent
    - replaces parent with node
  - something to transform nodes of one type to another
    - eg. dropout to gaussianfastdropout
  - replace_node
    - replace node with a new node
    - eg. replace input node with some shared variable container (StateNode?) for optimizing input wrt parameters as a visualiztion
- new parameter tags:
  - updates_state
...
- name all theano variables
  - when copying use theano.clone so that variables can be shared
- add more initializations
  - https://github.com/Lasagne/Lasagne/blob/master/lasagne/init.py
- have children container verify every element is of type NodeAPI
- figure out easy solution for theano.config.floatX everywhere
...
- function wrappers
  ...
  - padding w/ 0s for minibatch dim (to match input variable)
  - chunking numpy arrays
- workflows / hooks
  - training
  - distillation
