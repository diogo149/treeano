...
def pickle_network(network, dirname):
    ...
    with open(os.path.join(dirname, "root_node.pkl"), 'w') as f:
        pickle.dump(root_node, f, protocol=pickle.HIGHEST_PROTOCOL)
    with open(os.path.join(dirname, "value_dict.pkl"), 'w') as f:
        pickle.dump(value_dict, f, protocol=pickle.HIGHEST_PROTOCOL)


def unpickle_network(dirname):
    with open(os.path.join(dirname, "root_node.pkl")) as f:
        root_node = pickle.load(f)
    with open(os.path.join(dirname, "value_dict.pkl")) as f:
